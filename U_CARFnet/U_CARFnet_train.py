...
def U_CARFnetrain(batch=2, img_num=1448, epochs=50, load_model=None):
    myGene = trainGenerator(batch, "./data2/train", "image", "label", image_num=img_num)
    model = U_CARFnet(load_model)
    model_checkpoint = ModelCheckpoint('U_CARFnet_50.hdf5', monitor='loss', verbose=1, save_best_only=True)
